...
class JobMaster:
    def __init__(self):
        ...
        self.running_jobs = [
            RunningJob(job) for job in self.raw_jobs if job["JobState"] == "RUNNING"
        ]
        self.private_running_jobs = [
            job for job in self.running_jobs if job.is_private_gpu
        ]
        self.running_jobs = [job for job in self.running_jobs if not job.is_private_gpu]
        self.pending_jobs = [
            PendingJob(job) for job in self.raw_jobs if job["JobState"] == "PENDING"
        ]
        self.other_jobs = [
            OtherJob(job)
        ...
        self.user_running_jobs = [
            job for job in self.running_jobs if job.user_id == self.user
        ]
        self.user_pending_jobs = [
            job for job in self.pending_jobs if job.user_id == self.user
        ]
        self.user_other_jobs = [
            job for job in self.other_jobs if job.user_id == self.user
        ]
        self.user_running_jobs = sorted(self.user_running_jobs, key=lambda x: x.job_id)
        self.user_pending_jobs = sorted(self.user_pending_jobs, key=lambda x: x.job_id)
        self.user_other_jobs = sorted(self.user_other_jobs, key=lambda x: x.job_id)
    ...
    def get_gpus_on_node(self, node_name):
        jobwise_counts = [
            job.gpus for job in self.running_jobs if job.node == node_name
        ]
